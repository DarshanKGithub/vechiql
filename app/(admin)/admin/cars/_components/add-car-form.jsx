  ...
  const onMultiImageDrop = (acceptedFiles) => {
   const validFiles = acceptedFiles.filter((file) => {
    if(file.size > 5 * 1024 * 1024){
        toast.error(`${file.name} exceeds 5MB limit all will be skipped `);
        return false;
    }
    return true;
   });
        ...
        <Tabs defaultValue='ai' className='mt-6' value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value='manual' className='mt-6'>
              <Card>
                ...
                <div>
                    <Label htmlFor="images" className={imageEror ? " text-red-500" : ""}>
                        Images{""}
                        {imageEror && <span className='text-red-500'>*</span>}
                        </Label>
                    <div {...getMultiImageInputProps ()} className={`border-2 border-dashed rounded-lg p-6 text-center cursor-pointer hover:bg-gray-50 transition mt-2 ${imageEror ? "border-red-5000" : "border-gray-300"}`}>
                  <input {...getMultiImageRootProps()} />
